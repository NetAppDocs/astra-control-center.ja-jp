...
+
** VMware TKGおよびTKGiのデフォルト構成ファイルトークンは、デプロイ後10時間で有効期限が切れます。Tanzuポートフォリオ製品を使用する場合は、Astra Control Centerと管理対象アプリケーションクラスタ間の接続の問題を防ぐために、有効期限のないトークンを含むTanzu Kubernetes Cluster構成ファイルを生成する必要があります。手順については、 https://techdocs.broadcom.com/us/en/vmware-cis/nsx/vmware-nsx/4-2.html["VMware NSX-T Data Center 製品ドキュメント"^]をご覧ください。
