...
Astra Control Center をインストールするには、からインストールバンドルをダウンロードします https://mysupport.netapp.com/site/["ネットアップサポートサイト"^] 一連のコマンドを実行して、環境に Astra Control Center Operator と Astra Control Center をインストールします。この手順を使用して、インターネット接続環境またはエアギャップ環境に Astra コントロールセンターをインストールできます。
...
== Astra Control Center オペレータを設定します
...
----
. すべてのポッドが動作している場合は、 Astra Control Center Operator によってインストールされた AstraeControlCenter インスタンスを取得して、インストールが正常に完了したことを確認します。
+
[listing]
----
