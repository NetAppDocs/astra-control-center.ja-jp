...
NOTE: バケットを追加する際は、適切なバケットプロバイダーを選択し、そのプロバイダーの正しい認証情報を入力してください。例えば、UI は NetApp ONTAP S3 をタイプとして受け入れ、StorageGRID の認証情報を受け入れますが、これにより、このバケットを使用した今後のすべてのアプリのバックアップと復元が失敗するようになります。link:../release-notes/known-issues.html["リリースノート"]を参照してください。
