...
別の Astra Control Center インスタンスでクラスターを管理する場合は、別のインスタンスで管理する前に、現在管理しているインスタンスからまずlink:../use/unmanage.html#unmanage-a-cluster["クラスタの管理を解除します"]してください。クラスターを管理対象から削除した後、次のコマンドを実行して、クラスターが管理対象外になっていることを確認してください。
...
. https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy.html["最新バージョンのAstra Tridentをインストール"^]。
...
接続を確立する際は、必ずlink:../use/monitor-protect.html#add-a-proxy-server-for-connections-to-the-netapp-support-site["正しい値を入力します"]ようにしてください。
