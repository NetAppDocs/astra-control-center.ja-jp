...
+
** GKEクラスターの場合、Astra Control Serviceは https://www.netapp.com/google-cloud/netapp-volumes/["NetApp Cloud Volumes Service for Google Cloud"^]または Google Persistent Disksを永続ボリュームのストレージバックエンドとして使用します。
** AKSクラスターの場合、Astra Control Serviceは https://www.netapp.com/azure/azure-netapp-files/["Azure NetApp Files の特長"^]またはAzureマネージドディスクを永続ボリュームのストレージバックエンドとして使用します。
